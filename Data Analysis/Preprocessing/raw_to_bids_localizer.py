...
def prepare_data_for_mne_bids_pipeline(subject='01', 
                                       ...
                                       run_names=[f"{i:02}" for i in range(1, 12)]):
    """
    ...
    for run in run_names:
        ...
        data_path = original_data_path + f"sub-{subject}" + f'/run_localizer_{run}.fif'
...
prepare_data_for_mne_bids_pipeline(subject='01', base_path="../../Data/Pilot",
                                   task_name='localizer',
                                   run_names=[f"0{i}" for i in range(1, 3)])

subject = '01'
for run in [f'0{str(i)}' for i in range(1, 3)]:
    bids_root = "/Users/Barbu/Documents/Postdoc/Experiments/Pushmi/MEG/Data/Pilot/BIDS"
    bids_path = mne_bids.BIDSPath(subject=subject, datatype="meg", root=bids_root,task="localizer",run=run)
